...
public class DataGetRessourceController {
	...
	public ResponseEntity<?> getByNomAndPrenom(@PathVariable("nom") String nom,@PathVariable("prenom") String prenom){
		...
		List<Ressource> ressources = resService.getByNomAndPrenom(nom,prenom);
		List<RessourceDTO> ressourcesDTO = new ArrayList<>();
		for(Ressource r: ressources) {
			RessourceDTO dto = r.toDTO();
			dto.setDispositif(r.getEquipe().getDispositif().getNom());
			dto.setOrganisme(r.getEquipe().getDispositif().getOrganisme().getOrganisme());
			ressourcesDTO.add(dto);
		}
		return new ResponseEntity<>(ressourcesDTO,headers,HttpStatus.OK);
	}
	
	@GetMapping("/ByNomAndPrenomAndEquipe={nom}&{prenom}&{enom}")
	public ResponseEntity<?> getByNomAndPrenom(@PathVariable("nom") String nom,@PathVariable("prenom") String prenom,
		...
		headers.add("Access-Control-Allow-Headers", "X-Requested-With,content-type");
		headers.add("Access-Control-Allow-Credentials", "true");
		Ressource ressource = resService.getByNomAndPrenomAndEquipe(nom,prenom,enom);
