...
public class DataUpdateController {
	...
	public ResponseEntity<?> updateOrganisme(@RequestBody OrganismeDTO organisme){
		...
		List<Dispositif> dispositifs = dispositifService.getByOrganisme(organisme.getOrganisme());
		org.setDispositifs(dispositifs);
	...
	public ResponseEntity<?> updateRessource(@Valid @RequestBody RessourceDTO ressource){
		...
		Historiqueres h = histService.getByRessource(ressource.getNom(), ressource.getPrenom(),ressource.getEquipe());
		List<Ressourcehascompetence> rcs = rcService.getByRessource(ressource.getNom(), ressource.getPrenom(),ressource.getEquipe());
	...
	public ResponseEntity<?> updateHistorique(@Valid @RequestBody HistoriqueDTO hist){
		...
		Ressource r = resService.getByNomAndPrenomAndEquipe(hist.getRnom(), hist.getRprenom(), hist.getEquipe());
	...
	public ResponseEntity<?> updateRessourcehascompetence(@Valid @RequestBody RessourcehascompetenceDTO rcDTO){
		...
		Ressource res = resService.getByNomAndPrenomAndEquipe(rcDTO.getRnom(), rcDTO.getRprenom(), rcDTO.getEquipe());
	...
	@PutMapping("/updateDispositifhascompetence")
	public ResponseEntity<?> updateDispositifhascompetence(@Valid @RequestBody DispositifhascompetenceDTO dcDTO){
		HttpHeaders headers = new HttpHeaders();
		//headers.add("Access-Control-Allow-Origin", "*");
		headers.add("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT");
		headers.add("Access-Control-Allow-Headers", "X-Requested-With,content-type");
		headers.add("Access-Control-Allow-Credentials", "true");
		DispositifhascompetencePK id = new DispositifhascompetencePK(dcDTO.getIdd(), dcDTO.getIdc());
		Dispositifhascompetence rc = dcService.getById(id);
		Competence comp = compService.getByNom(dcDTO.getCompetence());
		Dispositif disp = dispositifService.getByNom(dcDTO.getDispositif());
		rc.setCompetence(comp);
		rc.setDispositif(disp);
		dcService.update(rc.getId(),rc);
		return new ResponseEntity<>(headers,HttpStatus.CREATED);
	}
